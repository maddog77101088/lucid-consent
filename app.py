...
- **vet_name (주치의)**: PMS365 상단 빨간 헤더의 우측에 흰 글씨로 표시된 한글 이름 (로그인한 수의사·직원). 보통 2-4글자 한글. 메뉴 아이콘이나 약어가 아닌 사람 이름. 예: "설재민", "노진희", "김소희". 없으면 빈 문자열.
  · 환자 정보의 "담당: XXX"(영문 약어, 예: WEVET)는 vet_name이 아니므로 무시.

**JSON 필드 전체 목록**: guardian_id, guardian_name, guardian_phone, guardian_mobile, guardian_address, guardian_rrn, animal_id, rfid, patient_name, species, breed, age, sex, coat_color, weight, vet_name
...
def _get_vet_list():
    """주치의 후보 목록 (role='vet' 또는 role='admin' 모두)."""
    db = get_db()
    return db.execute(
        "SELECT display_name FROM users WHERE role IN ('vet','admin') ORDER BY display_name"
    ).fetchall()
...
- 친근한 카톡 톤. 절대 길지 않게.
- 첫 줄: "🐾 [환자]이 보호자님,"
- 둘째 줄: 환자(반려동물) 상태를 묻는 자연스러운 한 문장. **보호자의 행위가 아닌 환자의 회복 상태**를 묻는 톤.
  · **⚠️ 압존법 주의**: 환자는 반려동물이므로 절대 존대 금지. "~신가요/~셨어요/~하시는지" 같은 환자 주어 존칭 표현 금지.
  · ✅ 좋은 예 (보호자에게 안부): "어제 진료 후 잘 지내셨나요?", "어제 수술 후 잘 지내셨어요?"
  · ✅ 좋은 예 (환자 주어, 평어체): "어제 수술 후 [환자] 컨디션은 어떤가요?", "어제 퇴원 후 [환자]가 잘 지내고 있나요?", "[환자] 회복은 어떤가요?"
  · ❌ 나쁜 예: "어제 진료 잘 마치셨나요?" (보호자가 진료를 마친 것처럼 어색)
  · ❌ 나쁜 예: "어제 수술 잘 받으셨나요?" (보호자가 수술받은 것처럼 어색)
  · ❌ 나쁜 예: "[환자] 컨디션은 어떠신가요?" (환자에게 존대 — 압존법 위반)
  · ❌ 나쁜 예: "[환자]가 잘 지내시나요?" (환자에게 존대 — 압존법 위반)
- 셋째 줄: "[환자]이가 잘 회복되고 있는지 궁금해요 😊" 같은 짧은 한 줄 (받침에 따라 "이가/가" 자연스럽게). 환자 주어 평어체 유지.
- 넷째 줄(빈 줄)
...
def _kakao_base_enabled():
    """솔라피 + PFID 기본 환경변수 (템플릿 무관)."""
    return all([
...
def _send_kakao_template(phone, template_id, variables):
    ...
    if not _kakao_base_enabled():
        return False, "카카오 발송 환경변수 미설정"
    if not template_id:
        return False, "템플릿 ID 누락"
...
def api_notice_send_kakao(doc_id):
    """안내문(CE/postop/imd)을 보호자에게 카카오 알림톡으로 발송 + 공유 토큰 생성."""
    if not _kakao_notice_enabled():
        return jsonify({"ok": False, "error": "안내문 카카오 발송 미설정 (KAKAO_TEMPLATE_ID_NOTICE 등)"}), 400
    data = request.get_json() or {}
    ...
    if not row:
        return jsonify({"ok": False, "error": "문서를 찾을 수 없음"}), 404
    ...
    notice_url = f"{_sign_base_url()}/notice/{token}"
    ok, result = _send_kakao_notice(
        phone=phone,
        patient_name=row["patient_name"],
        doc_type=row["doc_type"],
...
def notice_view(token):
    """보호자 공개 안내문 페이지 (로그인 불필요, 토큰 기반)."""
    db = get_db()
    row = db.execute(
        """SELECT doc_type, patient_name, guardian_name, diagnosis, body, vet_name, created_at
           FROM patient_documents WHERE share_token=?""",
        (token,)
    ).fetchone()
    if not row:
        return render_template("notice_view.html", error="유효하지 않은 링크입니다."), 404
    type_label = {
        "ce": "💬 진료안내문",
        "postop": "🏥 수술후 안내문",
        "imd": "🩺 내과 퇴원 안내문",
